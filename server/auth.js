...
// Brute-force throttle state (per email, in-memory).
...
export function authMiddleware(req, res, next) {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, email, name, provider, token_version, created_at FROM users WHERE id = ?').get(p.sub);
...
async function loginFail(res, email) {
  ...
  // Gentle, progressive delay blunts guessing without punishing humans.
  await new Promise((r) => setTimeout(r, Math.min(300 * f.count, 3000)));
  // Deliberately ambiguous: never reveal whether the email exists.
...
export function authRoutes(app) {
  ...
  app.post('/api/auth/register', async (req, res) => {
    ...
    db.prepare('INSERT INTO users (id, email, name, password_hash, provider, token_version, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .run(user.id, user.email, user.name, user.password_hash, 'email', 0, user.created_at);
  ...
  // Logout revokes the session server-side (token version bump), not just client-side.
  app.post('/api/auth/logout', (req, res) => {
    try {
      const token = req.cookies?.serene_token;
      if (token) {
        const p = jwt.verify(token, process.env.JWT_SECRET);
        db.prepare('UPDATE users SET token_version = token_version + 1 WHERE id = ?').run(p.sub);
      }
  ...
  // Account deletion requires the password: a stolen session alone can't wipe you.
  app.delete('/api/auth/account', authMiddleware, async (req, res) => {
    ...
    db.prepare('DELETE FROM users WHERE id = ?').run(req.user.id); // messages cascade-delete
    res.clearCookie('serene_token', { path: '/' });
    res.json({ ok: true });
  });
}
